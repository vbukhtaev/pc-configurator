...
import static ru.bukhtaev.controller.MainPowerConnectorRestController.URL_API_V1_MAIN_POWER_CONNECTORS;
...
class MainPowerConnectorRestControllerIT extends AbstractIntegrationTest {
    ...
    private NameableRequestDto connector20Pin;
    private NameableRequestDto connector24Pin;

    @BeforeEach
    void setUp() {
        connector20Pin = NameableRequestDto.builder()
                .name("20 pin")
                .build();
        connector24Pin = NameableRequestDto.builder()
                .name("24 pin")
                .build();
    }
    ...
    void getAll_shouldReturnAllEntities() throws Exception {
        // given
        repository.save(
                mapper.convertFromDto(connector20Pin)
        );
        repository.save(
        ...
        mockMvc.perform(requestBuilder)
                ...
                .andExpectAll(
                        ...
                        jsonPath("$", hasSize(2)),
                        jsonPath("$[0].name", is(connector20Pin.getName())),
    ...
    void getById_withExistentId_shouldReturnFoundEntity() throws Exception {
        // given
        final MainPowerConnector saved = repository.save(
                mapper.convertFromDto(connector20Pin)
        );
    ...
    void create_withNonExistentName_shouldReturnCreatedEntity() throws Exception {
        ...
        assertThat(repository.findAll()).hasSize(1);
        final String jsonRequest = objectMapper.writeValueAsString(connector20Pin);
        ...
        mockMvc.perform(requestBuilder)
                ...
                .andExpectAll(
                        ...
                        content().contentType(MediaType.APPLICATION_JSON),
                        jsonPath("$.name", is(connector20Pin.getName()))
                );
        ...
        assertThat(connector.getName()).isEqualTo(connector20Pin.getName());
    }
    ...
    void replace_withNonExistentName_shouldReturnReplacedEntity() throws Exception {
        ...
        final Optional<MainPowerConnector> optConnector = repository.findById(saved.getId());
    ...
    void update_withExistentName_shouldReturnError() throws Exception {
        ...
        assertThat(repository.findAll()).hasSize(1);
        final NameableRequestDto dto = NameableRequestDto.builder()
                .name(connector24Pin.getName())
                .build();

        final UUID connector20PinId = repository.save(
                mapper.convertFromDto(connector20Pin)
        ).getId();
        final String jsonRequest = objectMapper.writeValueAsString(dto);
        final var requestBuilder = patch(
        ...
        mockMvc.perform(requestBuilder)
                ...
                .andExpectAll(
                        ...
                        jsonPath("$.violations[0].message", is(
                                MessageFormat.format(
                                        "Main power connector with name <{0}> already exists!",
                                        connector24Pin.getName()
                                )
                        ))
                );

        final Optional<MainPowerConnector> optConnector = repository.findById(connector20PinId);
        assertThat(optConnector).isPresent();
        assertThat(optConnector.get().getName())
